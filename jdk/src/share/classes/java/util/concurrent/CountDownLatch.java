 ...
 * threads could pass.
 *
 * <p>下面这两个示例演示了CountDownLatch的典型用法。
        ...
        protected boolean tryReleaseShared(int releases) {
            ...
            for (;;) {
                ...
                if (c == 0)
                    return false;
                int nextc = c-1;
                // if CAS fail, retry in loop
                if (compareAndSetState(c, nextc))
                    // 当state减至0时，返回true。上层方法（即AbstractQueuedSynchronizer.releaseShared）就会调用doReleaseShared方法对CLH队列中的节点进行signal
                    return nextc == 0;
            }
        }
    }
